...
export const internalTokenMiddleware = (
  ...
  if (!header || !tokensMatch(header, RECOMMENDER_API_KEY)) {
    // #region agent log
    fetch('http://127.0.0.1:7430/ingest/c20838bf-9e24-484e-8317-a8bd52c8f7b2',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'d357df'},body:JSON.stringify({sessionId:'d357df',location:'internalAuth.middleware.ts:401',message:'recommender-export blocked: invalid token',data:{hasHeader:Boolean(header)},timestamp:Date.now(),hypothesisId:'H2'}),signal:AbortSignal.timeout(300)}).catch(()=>{});
    // #endregion
    return res
      .status(401)
  ...
  // #region agent log
  fetch('http://127.0.0.1:7430/ingest/c20838bf-9e24-484e-8317-a8bd52c8f7b2',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'d357df'},body:JSON.stringify({sessionId:'d357df',location:'internalAuth.middleware.ts:pass',message:'recommender-export auth ok',data:{},timestamp:Date.now(),hypothesisId:'H1'}),signal:AbortSignal.timeout(300)}).catch(()=>{});
  // #endregion
  return next();
};
